...
`APIRouter`; routes call into `netconfig/services/` (or the storage
...
`netconfig/main.py` adds its `/api/v1` prefix on top of each router's
...
`Depends(...)` factories defined in `netconfig/api/deps.py`.  Don't
...
Request and response shapes live in `netconfig/models/`.  Don't
...
shape, add it to `netconfig/models/` and import it.
...
`netconfig.api.routes.backups.get_collector`.  This is a **Hard
...
2. **Define request / response models** in `netconfig/models/`.
   Don't inline schemas in the route file.
3. **Implement service logic** in `netconfig/services/<concern>.py`
   (or in `netconfig/migration/` for translation work) when it's
...
5. **Update `netconfig/main.py`** to `include_router` the new
...
- `migration_pipeline.run_plan`, `run_plan_with_rename`,
  ...
  `netconfig/services/migration_pipeline.py` for the
