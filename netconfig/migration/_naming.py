...
See also:
- netconfig/migration/codecs/arista_eos/render.py — VLAN-name
  sanitisation already uses the same regex inline; the hostname
  emit now routes through this helper.
- netconfig/migration/codecs/cisco_iosxe_cli/render.py — hostname
  emit uses this helper.
- netconfig/migration/_user_secrets.py — sibling shared utility for
