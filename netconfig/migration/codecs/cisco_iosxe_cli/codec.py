...
class CiscoIOSXECLICodec(CodecBase):
    ...
    def classify_port_name(self, name: str):  # -> PortIdentity
        """Classify a Cisco IOS-XE port name into a vendor-agnostic
        :class:`PortIdentity`.

        Recognised forms (all case-insensitive):

        Physical interfaces:
            * ``<Prefix>X/Y`` — standalone (module / port), e.g.
              ``GigabitEthernet0/1``.
            * ``<Prefix>X/Y/Z`` — stacked (member / module / port),
              e.g. ``GigabitEthernet1/0/24``.
            * ``<Prefix>X/Y/Z/W`` — breakout child
              (member / module / port / lane), e.g.
              ``TenGigabitEthernet1/1/1/1`` under a ``FortyGigabitEthernet1/1/1``.

        Where ``<Prefix>`` encodes speed: ``FastEthernet`` (fast),
        ``GigabitEthernet`` (gig), ``TwoGigabitEthernet`` (2.5gig),
        ``FiveGigabitEthernet`` (5gig), ``TenGigabitEthernet`` (10gig),
        ``TwentyFiveGigE`` (25gig), ``FortyGigabitEthernet`` (40gig),
        ``HundredGigE`` (100gig), ``FourHundredGigE`` (400gig),
        ``AppGigabitEthernet`` (virtual app-hosting).
        ...
        Everything else → unknown (verbatim fallback + warning).
        """
        from ...canonical.port_names import PortIdentity

        # Physical port patterns — try 4-part (breakout), 3-part
        # (stacked), 2-part (standalone) in that order.  Case-
        # insensitive so "GigabitEthernet" and "gigabitethernet" both
        # match.
        m = re.match(
        ...
        if m:
            ...
            # bridge — same coordinates as a physical port but it's NOT
            # a physical port.  Preserve this as a meta hint so
            # format_port_identity can restore the Cisco-specific
            # prefix on same-vendor round-trip.  Cross-vendor targets
            # ignore the hint and drop back to their physical port
            # formatting (or their equivalent virtual concept).
    ...
    def format_port_identity(self, identity) -> str | None:
        ...
        Picks the speed-appropriate Cisco prefix for physical ports
        (using ``name_speed_hint`` with a ``GigabitEthernet`` fallback
        when the hint is empty or non-Cisco).  Logical kinds map to
        their canonical Cisco name.
        """
        ...
        if identity.kind == "breakout":
            ...
            # Use the *lane* speed prefix (e.g. 10gig for 4x10G under
            # a 40gig parent), falling back to the identity's hint.
            prefix = _CISCO_SPEED_TO_PREFIX.get(
