...
| SSH host-key not verified by default | A MITM on the management path could harvest the SSH password + enable secret on first connect | Yes (default) / opt-in hardening available | Default `ssh_host_key_checking=auto_add` trusts any key (legacy; assumes a trusted management VLAN).  Set `NETCANON_SSH_HOST_KEY_CHECKING=tofu` (record first key under `{data_dir}/known_hosts`, reject later changes) or `reject` (only known hosts) to harden.  Both collectors persist learned keys to `{data_dir}/known_hosts` and reject a later changed key — the Paramiko collector inline, the Netmiko collector via an auth-less paramiko pre-flight (`verify_host_key`) that reads + pins/rejects the host key before Netmiko connects `ssh_strict` against that store. |
