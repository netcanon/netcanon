...
def render_intent(tree: Any) -> str:
    ...
    # Inverse SVI fold: materialise ``interface Vlan<N>`` stanzas for
    # VLANs that carry L3 (``ipv4_addresses``) but have no sibling
    # ``Vlan<N>`` interface — the shape a Junos ``irb`` + ``l3-interface``
    # source produces.  Non-mutating: rendered alongside tree.interfaces
    # below, NOT appended to the (potentially reused) intent.  Empty for
    # same-vendor renders (the Vlan<N> interface is already present).
    ...
    for iface in [*tree.interfaces, *synth_svis]:
        has_renderable_attr = (
