        ...
        lossy=[
            ...
            LossyPath(
                ...
                reason=(
                    ...
                    "Lossy rather than supported because "
                    "``address-family ipv6`` / EVPN ``l2vpn evpn`` "
                    "sub-stanzas inside ``vrf definition`` are "
                    "parse-and-ignore in v1 (per-VRF static-route "
                    "membership DOES round-trip via the supported "
                    "``/routing/static-route/vrf`` surface).  Basic VRF + "
