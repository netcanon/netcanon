...
#: consume) lives in :mod:`netcanon.migration._user_secrets` and
...
def _split_aos_hash(hashed: str) -> tuple[str, str]:
    ...
    :mod:`netcanon.migration._user_secrets`.  Maps the shared
...
def render_intent(tree: Any) -> str:  # noqa: C901
    ...
    lines: list[str] = []
    lines.append(
        "; generated by netcanon translator (aruba_aoss codec)"
    )
    ...
    if tree.snmp is not None and (
        ...
        for u in tree.snmp.v3_users:
            ...
            if (u.auth_protocol or u.priv_protocol) and not user_usm_is_migratable(
                u, tree.source_vendor, "aruba_aoss",
            ):
    ...
    # :mod:`netcanon.migration._user_secrets` policy — operator
