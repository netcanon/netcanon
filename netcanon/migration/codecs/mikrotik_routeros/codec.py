    ...
    _CAPS: ClassVar[CapabilityMatrix] = CapabilityMatrix(
        ...
        lossy=[
            ...
            LossyPath(
                ...
                reason=(
                    "`authentication-password=` takes the operator's "
                    "passphrase and RouterOS derives the localised USM key "
                    "from it, so a value that is already another agent's key "
                    "cannot be re-derived: the render refuses it (review "
                    "comment, no `add name=` line) rather than deriving a key "
                    "from a key.  A source passphrase is portable and is "
                    "emitted unchanged."
                ),
                severity="warn",
            ),
            LossyPath(
                ...
                reason=(
                    "Same as the auth key: the encryption password is refused "
                    "with the user unless it is a portable passphrase or came "
                    "from RouterOS itself -- re-key the v3 user on the target."
                ),
                severity="warn",
            ),
            LossyPath(
        ...
        unsupported=[
            ...
            #    switchport surface above.  RouterOS creates a VLAN interface
            #    on a SINGLE parent (``/interface vlan interface=ether2``); the
            #    multi-port tagged/untagged membership a switch source carries
            #    lives in RouterOS's bridge VLAN table, which this codec does
            #    not model, so the port LISTS are dropped on render.  Declared
            #    unsupported so a switch→router migration surfaces the
            #    membership loss instead of reporting ``severity: ok`` while
            #    the walker yields these paths (silent-loss naming-sensitive
            #    guard). ──
            UnsupportedPath(
                path="/vlans/vlan/tagged-ports",
                reason="RouterOS binds a VLAN interface to a single parent; "
