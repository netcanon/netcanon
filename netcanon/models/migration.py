...
class CapabilityMatrix(BaseModel):
    ...
    #: guard — see ``netcanon.services.migration_validate.check_class_compat``.
...
class ValidationReport(BaseModel):
    ...
    Shape mirrors ``CompatibilityReport`` in ``netcanon.models.diff``
...
class TransformSpec(BaseModel):
    ...
    ``netcanon.migration.transforms`` registry resolves ``name`` to a
...
class MigrationJob(BaseModel):
    ...
    #: scope for this map (see :mod:`netcanon.migration.canonical.snmp_names`).
...
class CodecInfo(BaseModel):
    ...
    Attributes:
        input_format: Short catalogue tag from
            :data:`netcanon.migration.codecs.base.INPUT_FORMATS`
