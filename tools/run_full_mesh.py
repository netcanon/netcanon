...
# Fixture-directory -> codec mapping.  Imported from the netcanon
# package (NOT the pytest harness) so this script stays standalone while
# sharing one source of truth with test_real_captures.py — the two used
# to keep hand-replicated copies that drifted (see project memory).
...
def _list_drift_summary(
    ...
    if id_keys and src and isinstance(src[0], dict):
        ...
        for i, (s, t) in enumerate(zip(norm_src, norm_tgt, strict=False)):
            if s != t:
...
def render_matrix_md(result: dict[str, Any]) -> str:
    ...
    * Synthetic: "what survives translation when EVERY supported field
    ...
    lines.append(
        f"Generated by `tools/run_full_mesh.py` over {result['cells_total']} "
        f"cells ({real_fixtures_n} real + {synthetic_fixtures_n} synthetic "
        f"fixtures × {result['targets_count']} bidirectional targets).  "  # noqa: RUF001
        "Provenance is the git commit that updates this file, not a "
        "wall-clock stamp — so `--matrix` regeneration is byte-reproducible "
        "for a given tree (a non-empty `git diff` here means real drift, not "
        "a new run time)."
        "\n"
    )
    lines.append(
