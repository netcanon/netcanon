...
| VyOS | OSS Vyatta successor | **Codec shipped + `certified`** (`vyos`, bidirectional) — netcanon's 12th codec, a curly-brace `config.boot` parser (distinct from the set-form `juniper_junos`).  Real corpus under `tests/fixtures/real/vyos/`: 6 MIT `cisagov/prescup-challenges` (1.4-rolling-202105, IPv4/OSPF + IPv6/BGP) + 2 Apache-2.0 `zhouleyan/wcni-kind` (1.4-rolling-202307, `interfaces vxlan` tunnel pair).  Covered surfaces: interfaces / static / users / NTP (bare + block form) / `service snmp` / VRF / `interfaces vxlan`.  Still welcomed: a **second VyOS major** (1.3 / 1.5); a **permissive + curly-brace** real capture exercising **`service snmp`** or **`vrf name`** — a 2026-06 hunt found such configs only under GPL (`vyos-1x` smoketests) or unlicensed, so SNMP + VRF remain **synthetic-validated**; and `set`-form `config` input (deferred — needs Junos-probe disambiguation). |
