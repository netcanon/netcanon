...
| **dell_os10** | **0** | — (nothing committed) | **any permissively-licensed OS10 `show running-configuration` WITH ITS `! Version` BANNER INTACT.** Still the single highest-value ask in the table. The codec is validated against a 40-capture out-of-tree corpus spanning five OS10 releases (10.4.3.1 / 10.4.3.4 / 10.5.1.0 / 10.5.1.4 / 10.5.4.4) — 40/40 parse, 38/40 round-trip stable (the 2 exceptions are the credential gate correctly refusing a `$CREDENTIAL_PLACEHOLDER$` scrub token, not defects). ⚠️ The blocker is a near-perfect **inverse correlation**: the MIT-licensed captures all lack a version banner, while every capture that pins a release is unlicensed — so committing the licensed half alone would still miss the `certified` bar of ≥3 captures across ≥2 OS versions. Also wanted: **OS9 / FTOS** material (a different grammar this codec deliberately refuses), and configs that defeat the 500-byte probe window — jinja2 template headers, serial-console login banners, or QoS-leading openers (measured: 10 of 40 are claimed by `cisco_iosxe_cli` instead, 4 at ≥90) |
