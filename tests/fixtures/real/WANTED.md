...
| VyOS | OSS Vyatta successor | **Codec shipped + `certified`** (`vyos`, bidirectional) — netcanon's 12th codec, a curly-brace `config.boot` parser (distinct from the set-form `juniper_junos`).  A 6-config corpus from the MIT-licensed (SEI) `cisagov/prescup-challenges` (US President's Cup) is committed under `tests/fixtures/real/vyos/`: 2 round-1 routers (IPv4 + OSPF) + 4 round-3b routers (IPv6 + BGP), all VyOS 1.4-rolling.  Chosen over `vyos/vyos-build` to avoid the VyOS project's GPL/LGPL.  Additional grammar coverage welcomed for a **second VyOS major** (1.3 / 1.5 — the corpus is single-version), and for real captures exercising **`service snmp` / `vrf` / `interfaces vxlan`** (SNMP + VRF are synthetic-only today; VXLAN + set-form input are future phases). |
