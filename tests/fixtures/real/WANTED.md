...
| **dell_os10** | **9** | — (none states a release) | **a permissively-licensed OS10 `show running-configuration` WITH ITS `! Version` BANNER INTACT.** ⚠️ The ask is now NARROWER than it was: nine MIT-licensed captures landed 2026-09-23, so "any licence-clean OS10 capture" is **no longer** the gap. What is still missing is a licence-clean capture that *states its release*. The blocker is a near-perfect **inverse correlation**, verified against each repository's licence metadata rather than assumed: every source that pins a version (`segusaro/OS10_BGP_EVPN_ansible`, `dell-tsb/dell-mec`, `dell-tsb/dell-dme`, `install-safe-press/gb10-playbooks`) carries **no licence file at all**, while every MIT source (`canu`, `azsupport`, `azlocal`, `dellgeos`, `msnetcfg`) ships its configs banner-stripped. One versioned licence-clean capture promotes the codec to `certified`. Also wanted: **OS9 / FTOS** material (a different grammar this codec deliberately refuses, now also declined by `cisco_iosxe_cli`'s probe) |
