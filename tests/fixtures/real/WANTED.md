...
| Cisco IOS-XR | Service provider routing | **Codec shipped** (`cisco_iosxr`, bidirectional / `best_effort`) — the 7-config `batfish/lab-validation` corpus (Apache-2.0: `cisco_xr_ios_vpnv4`, `iosxr_ebgp_basic`, `iosxr_ibgp_rr_over_ospf`) is committed under `tests/fixtures/real/cisco_iosxr/`.  Additional grammar coverage welcomed for IS-IS, SR-MPLS, flex-algo, SRv6, L2VPN bridge-groups, ACL extended, and QoS class-maps — the highest-leverage pull-target is [`ios-xr/xrd-tools`](https://github.com/ios-xr/xrd-tools) (Apache-2.0, official Cisco GitHub org) `samples/xr_compose_topos/` (`simple-bgp`, `ospf-bgp-rr`, `isis-ipfrr`, `segment-routing` 8-node, `srv6-l3vpn`); 1-2 of these land in the Phase-4 `certified` push. |
