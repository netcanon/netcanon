...
Usage::
    ...
    from netcanon_desktop.icons import generate_tray_image, write_ico

    # PIL.Image for pystray
    tray_image = generate_tray_image()

    # Write .ico file to disk (required by pywebview for window/taskbar icon)
    ico_path = write_ico(Path(tempfile.gettempdir()) / "netcanon.ico")
"""
...
def write_ico(path: Path | None = None) -> Path:
    ...
    """
    if path is None:
        path = Path(tempfile.gettempdir()) / "netcanon.ico"
