"""
Entry point for ``python -m netcanon_desktop``.
...
def main() -> None:
    ...
    from netcanon_desktop.single_instance import acquire_singleton
...
def _configure_logging() -> None:
    ...
    Uses the frozen-vs-dev detection from ``netcanon_desktop.settings`` to
    decide whether to write a log file.  In frozen (installed) mode the log
    goes to ``%APPDATA%\\Netcanon\\netcanon.log``; in dev mode only the
    ...
    if getattr(sys, "frozen", False):
        ...
        log_file = base / "Netcanon" / "netcanon.log"
    else:
