...
| [`static_routes.md`](static_routes.md) | FortiOS dotted-mask + outgoing-interface -> Junos CIDR + qualified-next-hop.  Default-VRF round-trips; per-VRF unsupported (FortiGate codec does not parse `set vrf <id>`; Junos render does not emit per-VRF statics, though `CanonicalStaticRoute.vrf` exists). |
