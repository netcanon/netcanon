...
That's the entire parse surface.  `intent.hostname`, `intent.snmp`,
...
`intent.timezone`, `intent.domain` are NEVER populated by this codec,
regardless of what the device's NETCONF reply carries.  Even
`intent.source_version` stays empty (no version-hint extraction).
...
vxlan_vnis = []                # never set (matrix declares unsupported)
...
`ipv4_addresses`, `ipv6_addresses` (scope=global hard-coded; see
`ipv6-addresses.md`).
...
Concretely, this means: the Junos target render running on a
...
fields — most of which the Junos target already supports cleanly.
