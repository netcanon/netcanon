# Dell OS10 → AOS-CX: measured canonical surface

Cached measurement backing
`tests/fixtures/cross_vendor_expectations/dell_os10__aruba_aoscx.yaml`.
...
|---|---|---|---|---|
| `anycast_gateway_mac` | not_applicable | 0 | 0 | 1 |
...
| `routing_instances[].description` | unsupported | 0 | 0 | 1 |
...
> Render emits no name-server config; intent.dns_servers are dropped on migration.
...
> AOS-CX declares no IANA ifType; the codec infers it from the interface-name shape (`1/1/1` -> ethernetCsmacd, `vlan N` -> l3ipvlan, `lag N` -> ieee8023adLag, `loopback N` -> softwareLoopback, `mgmt` -> ethernetCsmacd, `vxlan N` -> tunnel).  Inference is best-effort and may not catch every IANA type.
...
Sample: `{"lags[0] {'name': 'port-channel100'}": {'mode': {'source': 'active', 'target': 'static'}, 'name': {'source': 'port-channel100', 'target': 'lag 100'}}, "lags[1] {'name': 'port-chan`
...
> Render emits no NTP config; intent.ntp_servers are dropped on migration.
...
### `routing_instances[].description` — unsupported

Drifted on 0 of 1 cells (preserved on 0, both sides empty on 1).

> The AOS-CX `vrf <name>` stanza is a bare name in v1; VRF descriptions / RD / route-target live under the deferred `evpn` / `router bgp` blocks.

### `snmp.trap_hosts` — unsupported

Drifted on 1 of 1 cells (preserved on 0, both sides empty on 0).

> The `snmp-server host <ip> trap version ... community ...` trap-receiver grammar is deferred; v2c community + system-location / system-contact + v3 USM users are supported.
...
Sample: `count drift: 3 → 1 (static_routes)`
