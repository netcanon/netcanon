# SNMP: Cisco IOS-XE versus Arista EOS
...
Round-trip is good for v1 / v2c.  v3 USM lands cross-vendor for the
fields modelled on `CanonicalSNMPv3User` (name, group, auth_protocol,
auth_passphrase, priv_protocol, priv_passphrase, engine_id) BUT the
opaque hashed passphrases are NOT cross-compatible: Cisco salts USM
keys with one vendor-specific engineID-derived constant, Arista with
a different one.  Cross-vendor migration of v3 users requires re-
