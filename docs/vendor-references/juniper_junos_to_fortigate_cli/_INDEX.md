...
| [`static_routes.md`](static_routes.md) | Junos CIDR + qualified-next-hop -> FortiGate dotted-mask + outgoing-interface.  Default-VRF round-trips; per-VRF unsupported (Junos codec does not harvest per-VRF statics; FortiGate render does not emit per-VRF form, though `CanonicalStaticRoute.vrf` exists). |
