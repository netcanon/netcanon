# Dell OS10 → FortiGate: measured canonical surface

Cached measurement backing
`tests/fixtures/cross_vendor_expectations/dell_os10__fortigate_cli.yaml`.
...
Across the 1 cell, 14 source interface records produce 15 re-parsed records — the interface inventory is intact.
...
> FortiOS has no IANA ifType; inferred from 'type vlan' sub-setting or name shape.
...
> FortiOS binds a VLAN to a single parent child-interface; multi-port tagged membership (switchport's VLAN-centric twin) is dropped on render.

### `vlans[].untagged_ports` — unsupported

Drifted on 1 of 1 cells (preserved on 0, both sides empty on 0).

> FortiOS binds a VLAN to a single parent child-interface; multi-port untagged membership (switchport's VLAN-centric twin) is dropped on render.
