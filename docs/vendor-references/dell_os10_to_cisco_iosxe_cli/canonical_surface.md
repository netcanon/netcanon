# Dell OS10 → Cisco IOS-XE: measured canonical surface

Cached measurement backing
`tests/fixtures/cross_vendor_expectations/dell_os10__cisco_iosxe_cli.yaml`.
...
> CLI parser infers interface type from the name prefix (GigabitEthernet → ethernetCsmacd, Loopback → softwareLoopback) but cannot detect all IANA types.
...
Sample: `{"lags[0] {'name': 'port-channel100'}": {'name': {'source': 'port-channel100', 'target': 'Port-channel100'}}, "lags[1] {'name': 'port-channel200'}": {'name': {'source': 'port-chann`
