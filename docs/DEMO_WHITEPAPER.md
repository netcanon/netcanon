...
- `sendBeacon` on `pagehide` is best-effort; when it doesn't fire, the
  ...
  after it was created** (older than
  `HARD_TTL + POOL_MAX_AGE + 120 s slack = 1320 s`, plus up to one 60 s sweep
  interval). A crash can therefore widen the ceiling from 15 to at most ~23
  minutes — it can never *lift* it; nothing keeps your instance alive
  indefinitely.
