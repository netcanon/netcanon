...
| `cisco_nxos`      | Cisco NX-OS  | `show running-config` text | bidirectional | best_effort (all 4 phases — L1/L3 + L2 switchport/LAG + SNMP/users + HSRP + VRF RD/RT + per-VRF static + VXLAN-EVPN/L3VNI + IPv4 Distributed Anycast Gateway; only IPv6 anycast + Tier-3 remain unsupported) |
