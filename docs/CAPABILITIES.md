...
| `aruba_aoscx`     | Aruba AOS-CX  | `show running-config` text | bidirectional | experimental (Phases 1-2 — hostname + interfaces (multi-token names `interface vlan 11` / `lag 1` / `1/1/1`; L3 + L2 switchport `no routing` / `vlan access` / `vlan trunk`) + VLANs (id/name/description + port projection) + LAGs (`interface lag` + `lacp mode`) + local users + top-level `vrf` + default-VRF static; SNMP / `active-gateway` anycast / VXLAN-EVPN follow in later phases) |
