...
| `vyos`            | VyOS          | `config.boot` curly-brace text | bidirectional | certified (Phases 1-5 — `system host-name` + ethernet / loopback / dummy interfaces (address IPv4+IPv6 CIDR / `dhcp` / description / `disable` / mtu) + `vif` VLAN sub-interfaces (`ethN.<vid>`) + `protocols static` routes + `system login` local users + `system`/`service` ntp servers (bare + 1.4 block form) + `bonding` LAGs (`mode 802.3ad` LACP + both member forms) + `service snmp` (v1/v2c community / location / contact + v3 USM users) + VRF (`vrf name` routing-instances + per-interface `vrf` binding) + `interfaces vxlan` netdevs (one VNI each — vni / source / mcast or remote / port); round-trip-validated against a 6-config `cisagov/prescup-challenges` (MIT, IPv4/OSPF + IPv6/BGP) corpus + a 2-config `zhouleyan/wcni-kind` (Apache-2.0) VXLAN tunnel pair; curly-brace `config.boot` form — distinct from the `set`-form `juniper_junos` codec; per-VRF static routes + symmetric-IRB L3VNI + `set`-form input remain later phases) |
