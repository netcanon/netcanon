...
| `vyos`            | VyOS          | `config.boot` curly-brace text | bidirectional | experimental (Phases 1-3 — `system host-name` + ethernet / loopback / dummy interfaces (address IPv4+IPv6 CIDR / `dhcp` / description / `disable` / mtu) + `vif` VLAN sub-interfaces (`ethN.<vid>`) + `protocols static` routes + `system login` local users + `system`/`service` ntp servers + `bonding` LAGs (`mode 802.3ad` LACP + both member forms) + `service snmp` (v1/v2c community / location / contact + v3 USM users) + VRF (`vrf name` routing-instances + per-interface `vrf` binding); curly-brace `config.boot` form — distinct from the `set`-form `juniper_junos` codec; per-VRF static routes + VXLAN + the certified tier follow in later phases) |
