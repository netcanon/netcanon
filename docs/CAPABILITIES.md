...
| `aruba_aoscx`     | Aruba AOS-CX  | `show running-config` text | bidirectional | best_effort (Phases 1-3 — hostname + interfaces (multi-token names `interface vlan 11` / `lag 1` / `1/1/1`; L3 + L2 switchport `no routing` / `vlan access` / `vlan trunk`) + VLANs (id/name/description + port projection) + LAGs (`interface lag` + `lacp mode`) + local users + SNMP (community / system-location / system-contact / v3 USM) + `active-gateway` anycast (VSX/EVPN distributed gateway) + top-level `vrf` + default-VRF static; VXLAN-EVPN + a real-capture corpus (certified tier) follow in later phases) |
