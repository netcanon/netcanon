...
| `aruba_aoscx`     | Aruba AOS-CX  | `show running-config` text | bidirectional | best_effort (Phases 1-2b — hostname + interfaces (multi-token names `interface vlan 11` / `lag 1` / `1/1/1`; L3 + L2 switchport `no routing` / `vlan access` / `vlan trunk`) + VLANs (id/name/description + port projection) + LAGs (`interface lag` + `lacp mode`) + local users + SNMP (community / system-location / system-contact / v3 USM) + top-level `vrf` + default-VRF static; `active-gateway` anycast / VXLAN-EVPN + a real-capture corpus (certified tier) follow in later phases) |
