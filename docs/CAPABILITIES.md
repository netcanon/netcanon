...
| `aruba_aoscx`     | Aruba AOS-CX  | `show running-config` text | bidirectional | experimental (Phase 1 — hostname + basic-L3 interfaces (multi-token names: `interface vlan 11` / `lag 1` / `1/1/1`) + VLANs (id/name/description) + top-level `vrf` + default-VRF static routes; L2 switchport / LAG / SNMP / users / `active-gateway` anycast / VXLAN-EVPN follow in later phases) |
